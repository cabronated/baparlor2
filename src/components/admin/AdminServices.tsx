...
export default function AdminServices() {
  ...
  const predefinedCategories = ['Skin & Beauty', 'Hair Services', 'Makeup Services', 'Nail Services'];
      ...
      {services.length === 0 ? (
        ...
        <div className="grid grid-cols-1 overflow-hidden border border-brand-black/10 bg-white">
          ...
          {services.map((service, i) => (
            <div key={service.id} className={`grid grid-cols-12 gap-4 p-4 items-center ${i !== services.length - 1 ? 'border-b border-brand-black/5' : ''} hover:bg-brand-ivory/10 transition-colors`}>
